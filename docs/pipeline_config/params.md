...
A Nextflow pipeline for single cell RNA-seq analysis built on utia-gc/ngs.
...
| `exploratoryTag` | Tag to add to the end of the exploratory directory name to describe what's being explored. Use in conjunction with the `exploratory` profile. | `string` |  |  |  |
